...
from ..config import CONFIG, logger, YOLO_AVAILABLE, AVAILABLE_POSE_MODELS, PIL_AVAILABLE
...
class TrainingTab:
    ...
    def _ui(self, fn):
        """Run UI update safely from other threads."""
    ...
    def _device_to_ultralytics(self, device_str: str):
        """
        Ultralytics najlepiej przyjmuje:
        - 'cpu'
        - int GPU index (0,1,2...)
        - 'auto' obsługujemy sami
        """
        if not device_str:
            return "auto"
        if device_str == "auto":
    ...
    def _build_ui(self):
        # main notebook: Training / Dataset
    ...
    def _build_train_tab(self):
        # Horizontal splitter: left controls / right history+plots
    ...
    def _build_plots_ui(self):
        ...
        # --- prawa strona: zoom + canvas + scrollbary ---
    ...
    def _build_dataset_tab(self):
        # Vertical splitter: creator / splitter
    ...
    def _build_creator_ui(self):
        ...
        ttk.Scale(ratios, from_=50, to=95, variable=self.train_pct,
                  command=lambda e: self._update_ratio_labels()).grid(row=0, column=1, sticky=tk.EW, padx=5)
        self.train_lbl = ttk.Label(ratios, text="80%")
        self.train_lbl.grid(row=0, column=2, sticky=tk.W)

        ttk.Label(ratios, text="Val %").grid(row=1, column=0, sticky=tk.W)
        self.val_pct = tk.DoubleVar(value=20.0)
        ttk.Scale(ratios, from_=5, to=50, variable=self.val_pct,
                  command=lambda e: self._update_ratio_labels()).grid(row=1, column=1, sticky=tk.EW, padx=5)
        self.val_lbl = ttk.Label(ratios, text="20%")
        self.val_lbl.grid(row=1, column=2, sticky=tk.W)

        self.use_test = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            ratios, text="Utwórz test (reszta %)",
            variable=self.use_test, command=self._update_ratio_labels
        ).grid(row=2, column=0, columnspan=2, sticky=tk.W)
        self.test_lbl = ttk.Label(ratios, text="Test: 0%")
        self.test_lbl.grid(row=2, column=2, sticky=tk.W)
    ...
    def _build_splitter_ui(self):
        ...
        ttk.Scale(ratios, from_=50, to=90, variable=self.split_train,
                  command=lambda e: self._update_split_labels()).grid(row=0, column=1, sticky=tk.EW, padx=5)
        self.split_train_lbl = ttk.Label(ratios, text="70%")
        self.split_train_lbl.grid(row=0, column=2, sticky=tk.W)

        ttk.Label(ratios, text="Val %").grid(row=1, column=0, sticky=tk.W)
        self.split_val = tk.DoubleVar(value=20.0)
        ttk.Scale(ratios, from_=5, to=40, variable=self.split_val,
                  command=lambda e: self._update_split_labels()).grid(row=1, column=1, sticky=tk.EW, padx=5)
        self.split_val_lbl = ttk.Label(ratios, text="20%")
        self.split_val_lbl.grid(row=1, column=2, sticky=tk.W)

        self.split_use_test = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            ratios, text="Test (reszta %)",
            variable=self.split_use_test, command=self._update_split_labels
        ).grid(row=2, column=0, columnspan=2, sticky=tk.W)
        self.split_test_lbl = ttk.Label(ratios, text="Test: 10%")
        self.split_test_lbl.grid(row=2, column=2, sticky=tk.W)
    ...
    def _on_base_model_change(self):
        ...
        if key in AVAILABLE_POSE_MODELS:
            ...
            txt = (
                f"{info.get('name', key)}\n"
                f"Parametry: {info.get('params','?')} | Szybkość: {info.get('speed','?')}\n"
    ...
    def _start_training(self):
        ...
        if not dataset:
            messagebox.showerror("Błąd", "Wybierz dataset.")
            return
        ...
        def on_epoch_end(epoch, metrics):
            def upd():
                ...
                self.progress_label.configure(
                    text=f"Epoka {epoch}/{run.epochs} | mAP50={metrics.get('map50',0):.3f}"
                )
            self._ui(upd)
    ...
    def _on_run_selected(self, event=None):
        ...
        if plots_dir.exists():
            paths = TrainingReportGenerator.find_plots(plots_dir)
        else:
            paths = TrainingReportGenerator.find_plots(train_dir)
    ...
    def _change_zoom(self, factor: float):
        ...
        if self._plot_original_path:
            self._show_plot(Path(self._plot_original_path), force=True)

    def _show_plot(self, img_path: Path, force: bool = False):
        if not PIL_AVAILABLE:
    ...
    # ============================================================
    # Dataset Splitter (existing YOLO dataset)
    # ============================================================
    ...
    def _split_dataset_thread(self):
        ...
        def worker():
            ...
            ok, msg, stats = self.splitter.split_dataset(
                source_dir=src,
                output_dir=out,
                ratios=ratios,
                progress_callback=progress
