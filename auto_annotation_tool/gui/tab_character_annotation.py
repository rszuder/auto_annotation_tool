...
class CharacterAnnotationTab:
    ...
    def _run_extraction(self):
        ...
        if not xml_path.exists() or not images_dir.exists(): return messagebox.showerror("Błąd", "Brak plików wejściowych!")
        ...
        def worker():
            ...
            except Exception as e: self._log(self.ext_log, f"\n❌ BŁĄD: {e}\n", "ERROR")
            finally:
                self.is_processing = False
    ...
    def _open_filter_lab(self):
        ...
        if not self.preview_plate_ids: return messagebox.showinfo("Brak", "Wczytaj najpierw listę tablic.")
    ...
    def _build_cvat_tab(self, parent):
        ...
        ttk.Label(import_lf, text="Wskaż plik annotations.xml pobrany z CVAT (w formacie CVAT 1.1),\naby nadpisać błędne odczyty z JSON-a perfekcyjnymi poprawkami człowieka.", foreground="gray").pack(anchor=tk.W, pady=(2, 8))
    ...
    def _run_cvat_export(self):
        ...
        try:
            ...
            if self.smart_export_var.get():
                ...
                with open(work_dir / "temp_meta.json", 'w') as f: json.dump(filtered, f)
                source_meta = work_dir / "temp_meta.json"
            else: source_meta = meta_path
            ...
            if CVATCharacterExporter().export(source_meta, out_xml):
                ...
                msg = (
                    f"✅ SUKCES: Wygenerowano paczkę ZIP!\n\n"
                    f"Plik został zapisany tutaj:\n{out_zip}\n\n"
                    f"Wgraj go do CVAT, popraw czerwone ramki i pobierz zaktualizowany plik XML, aby wgrać go obok (Import)."
                )
                self._set_console_text(self.export_console, msg)
                
            if self.smart_export_var.get() and source_meta.exists(): source_meta.unlink()
                
        except Exception as e: 
            self._set_console_text(self.export_console, f"❌ BŁĄD EKSPORTU CVAT:\n{e}")

    def _run_yolo_gold_export(self):
        self._set_console_text(self.export_console, "⌛ Zbieranie idealnych tablic ze wszystkich folderów...")
        ...
        try:
            ...
            for run_dir in base_chars_dir.iterdir():
                ...
                with open(meta, 'r') as f: metadata = json.load(f)
                ...
                for pid, data in {k: v for k, v in metadata.items() if v.get("status") == "perfect"}.items():
                    ...
                    for c in data.get("characters", []):
                        ...
                        xc, yc = max(0.0, min(1.0, ((x1+x2)/2.0)/iw)), max(0.0, min(1.0, ((y1+y2)/2.0)/ih))
                        w, h = max(0.0, min(1.0, (x2-x1)/iw)), max(0.0, min(1.0, (y2-y1)/ih))
                        txt.append(f"{char_map[ch]} {xc:.6f} {yc:.6f} {w:.6f} {h:.6f}")
                    
                    with open(lbl_out / f"{new_pid}.txt", "w") as f: f.write("\n".join(txt))
                    copied += 1
            ...
            msg = (
                ...
                f"----------------------------------------------------\n"
                f"Skopiowano fizyczne zdjęcia (.jpg) dla w 100% poprawnych tablic: {copied} szt.\n\n"
                f"📁 LOKALIZACJA ZBIORU:\n"
                f"Workspace/4_training_datasets/{yolo_out.name}\n"
                f" ├── 📄 data.yaml (konfiguracja 36 klas: A-Z, 0-9)\n"
                f" ├── 📁 images/   (zawiera wszystkie {copied} skopiowanych zdjęć)\n"
                f" └── 📁 labels/   (zawiera pliki .txt z gotowymi ramkami znaków)\n\n"
                f"🚀 CO ROBIĆ DALEJ?\n"
                f"1. Przejdź do zakładki 'Trening i Analiza' -> 'Podział Datasetu (Splitter)'.\n"
                f"2. Wskaż ten folder, by program rozdzielił zdjęcia na katalogi Train / Val.\n"
                f"3. Przejdź do pod-zakładki 'Trening', załaduj podzielony zbiór i wyucz swój model OCR!"
            )
            self._set_console_text(self.export_console, msg)
    ...
    def _run_cvat_import(self):
        ...
        try:
            ...
            for image in root.findall('.//image'):
                ...
                for box in image.findall('box'):
                    if box.get('label', '').lower() in ['character', 'char']:
                        ...
                        new_chars.append({"character": t, "bbox": [float(box.get('xtl', 0)), float(box.get('ytl', 0)), float(box.get('xbr', 0)), float(box.get('ybr', 0))], "confidence": 1.0, "method": "cvat_manual"})
            ...
            msg = (
                f"✅ BAZA DANYCH ZAKTUALIZOWANA!\n"
                f"Zastąpiono stare błędy OCR poprawionymi danymi.\n"
                f"Nadpisano: {updated} tablic.\n\n"
                f"Są one teraz gotowe do eksportu do Datasetu YOLO!"
            )
            self._set_console_text(self.import_console, msg)
