...
class ParticipantProfilePanel:
    def __init__(self, parent, *, palette=None):
        ...
        ttk.Label(self.frame, textvariable=self.title, font=("Segoe UI", 10, "bold")).grid(
        ...
        for column, (title, fields) in enumerate(groups):
            ...
            ttk.Label(group, text=title, font=("Segoe UI", 9, "bold")).grid(
                row=0, column=0, columnspan=2, sticky="w", pady=(0, 4))
            for index, (key, caption) in enumerate(fields, 1):
                caption_widget = self.captions[key] = ttk.Label(group, text=caption, foreground=self.palette["muted"])
                caption_widget.grid(row=index, column=0, sticky="w", padx=(0, 8), pady=2)
                ...
                if key in {"run", "dataset"}:
                    ...
                    widget = ttk.Label(group, textvariable=var,
